...
`Option<T>` 的一层包装不是多余数据,而是把“有值/没值”放进类型系统:

```text
T           -> 使用时一定有值
Option<T>   -> 使用前必须处理 Some(T) 和 None
```

常见处理方式:

```rust
let value: Option<String> = Some("main".to_string());
...
// None 在当前函数中不能接受,转成 Result
let text = value.as_ref().ok_or("missing value")?;
```
...
| 方法 | 输入与输出 | 作用 |
| --- | --- | --- |
| `as_ref()` | `&Option<T> -> Option<&T>` | 借用内部值,不把 `T` 移出 |
| `as_mut()` | `&mut Option<T> -> Option<&mut T>` | 可变借用内部值 |
| `as_deref()` | `&Option<T> -> Option<&T::Target>` | 借用后再自动解引用,如 `Option<String> -> Option<&str>` |
| `map(f)` | `Option<T> -> Option<U>` | 只在 `Some` 时转换内部值 |
| `and_then(f)` | `Option<T> -> Option<U>` | 串联可能继续返回 `None` 的操作 |
| `unwrap_or(v)` | `Option<T> -> T` | `None` 时使用已给默认值 |
| `unwrap_or_else(f)` | `Option<T> -> T` | `None` 时才计算默认值 |
| `ok_or(e)` | `Option<T> -> Result<T, E>` | 把缺失升格为错误 |

选择方式的关键问题:

```text
None 是正常结果?       -> 保留 Option
None 在当前用例中是错误? -> ok_or / ok_or_else
只想看内部值不想移动? -> as_ref / as_deref
需要改变内部值?       -> as_mut 或取出后重新放回
```
...
- GitTributary 的 `AppState.repo` 使用 `Option<GitRepo>` 表达应用可能尚未打开仓库。`None` 在全局状态中是合法初始状态,但对 `get_status` 这类要求仓库的 command 而言必须转成错误。
- `lock.as_ref().ok_or("尚未打开仓库")?` 分两步:`as_ref` 借用 `GitRepo`,`ok_or` 把 `None` 转成 `Err`。
- 并发组合: [Mutex 与共享状态并发](../16.无畏并发/16.3.Mutex与共享状态并发.md)
