...
`'_` 是“请编译器在这个位置推断生命周期”,它不表示 `'static`,也不是忽略生命周期安全检查。
...
fn print_view(view: View<'_>) {
    println!("{}", view.value);
}
```

`View<'_>` 表示 `View` 内部确实存在一个借用,但当前位置不需要给它命名为 `'a`。编译器仍然会检查 `View` 不能活得比被引用的字符串更久。

什么时候需要命名生命周期:

```text
只是声明“这里有一个借用”       -> `'_` 或省略
需要表达多个引用之间的约束关系 -> 命名为 `'a`、`'b`
需要表达整个程序期间都有效       -> `'static`
```
...
- Tauri 的 `State<'_, AppState>` 可读成“一个内部借用 `AppState` 的状态包装器,借用期限由编译器在 command 参数位置推断”。
- `'_` 不是 Tauri 专属语法;Tauri 只是使用了 Rust 的匿名生命周期。
- 框架知识: [Tauri State 与应用状态](../../../全栈工程/Tauri/核心概念/State%20与应用状态.md)
