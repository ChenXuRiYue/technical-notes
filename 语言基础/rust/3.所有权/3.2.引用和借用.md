...
fn inspect_text(value: &str) {}
fn modify(value: &mut String) {}
```

| 参数 | 函数获得的权限 | 调用后原变量 |
| --- | --- | --- |
| `String` | 获得所有权,可以修改或销毁 | 通常已移动,不能继续使用 |
| `&String` | 共享只读借用 | 借用结束后可继续使用 |
| `&str` | 共享只读的字符串切片 | 借用结束后可继续使用 |
| `&mut String` | 独占可变借用 | 可变借用结束后可继续使用 |

读项目代码时,可以先不管函数体,仅根据签名回答:
...
`&String` 可以通过 deref coercion 自动转成 `&str`;`&Wrapper<T>` 也可在 `Wrapper<T>: Deref<Target = T>` 时自动转成 `&T`。这是一些框架包装器能直接传给接收内部类型引用的原因。
...
- GitTributary command 的 `String` 参数从 IPC 边界进入后由函数拥有;向 `GitRepo::open(&path)` 或 `commit_with_identity(&message, &identity)` 传递时只是短暂借用。
- `&state` 能传给接收 `&AppState` 的 helper,是因为 Tauri `State` 包装器可解引用到内部状态。
