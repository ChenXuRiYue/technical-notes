...
1. 变量其实就是所有权吗？
   ...
   变量是所有者，所有权代表的是 变量和值之间的一套关系

2. 如果一个函数传一个所有权参数，那么原所有者丢失了对于这个值的所有权。这种情况看上去有点诡异

   Rust 中提供了借用、克隆等方式传参

3. 为什么有引用不照搬 Java、C++ 那一套所有权引用？
   其实主要为了解决内存释放的问题：
   C++：释放申请的内存必须手动调用 new 和 delete
   Java：引入了垃圾回收机制，带来运行时开销，具有性能上线
   Rust： 引入了一套所有权，编译器可以清楚知道每一个值什么时候申请，什么时候分配，什么时候释放
