...
- GitTributary command 通常返回 `Result<T, String>`,使 IPC 边界可以把失败传给前端。业务 crate 的结构化错误会在边界使用 `.map_err(|e| e.to_string())` 转换。
- `repo.status().map_err(|e| e.to_string())` 直接返回整个 `Result`;`repo.stage_all().map_err(|e| e.to_string())?` 则在成功时继续后续步骤,失败时提前返回。
- 边界判断:字符串错误适合传输,但不应过早在核心业务层丢失错误分类。
