...
3. 分析元凶
   ...
   **谁在用它？** 往上看堆栈，报错的 Bean 是
      ...
      com.axel.demo.xxxxcommonutil.aop.aspect.TraceMapperAspect。

4. 结论：项目中有一个切面类 TraceMapperAspect，它依赖了 AspectJ 的库，但是项目里没有引入 AspectJ 的 Jar 包



>💭 缺 jar 包为什么没在编译期发现？
>
>编译器（javac）只检查 “语法” 和 “类型”
>当执行 mv n complie 时，或 IDE 编译代码时，流程为：
>
>1. 语法检查：代码是否写错，如少个分号，拼写错误。
>2. 类型检查：引用的类是否存在，以及调用的方法是否存在于该类中。
>
>上述出现报错其实是由这几个直接原因造成：
>
>1. IDEA 中的检错仅对工作区代码检错。
>2. 实现的项目引入了一个 Common 包，该 Common 包基于 provide 引入了 aop (provided 控制该 jar 包仅在编译期可见，在运行期不可见，所以运行时会报错)
...
4. 结论
   ...
   3.x 才有的类结构，但项目环境是 Spring Boot 2.x，导致找不到类。
