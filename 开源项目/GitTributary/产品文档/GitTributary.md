...
2. 最小权限及安全原则实践
3. 个人信息脱敏 （隐私信息脱敏化，加密形成独立数据库）
4. 日记加密插件

#### 🔖  监听

1. 监听目录的增删改查，针对 typora，vim，txt 等偏文本创作，弱功能化的软件做增强。
...
5. 定制化文件打标：为累积类的文件（如工具书，积累类文件）设置阈值，达到阈值时触发后续元工作流。（避免积累类笔记频繁修改及提交）

6. 对当前工作区备份

   > 一次因为工具误用，把所有 changes 都 discard 了。很难找回来
   
7. vibe coding：所见即所得。

   > 简单的在页面中回答，然后生成插件以及页面渲染在 APP 中
...
1. obsidian 下，难以表达 gitnoter 的git。因为 obsidian 会有非常多的数据产物，这为obsidian 中 带来增强，但这些都会被 git 跟踪，会影响笔记库 git history 本身的一种纯粹感。不侵入系统，应该对这些积累产生的文件存储在另外一个 git 库中，防止污染主笔记库的 git history
...
https://www.u-tools.cn/docs/developer/utools-api/simulate.html
